  ...
  async processCommentBatch(commentBatch, results) {
    ...
    try {
      ...
      if (tokenLimitError && commentBatch.length > 1) {
        ...
        const firstResult = await this.processCommentBatch(firstHalf, results);
        const secondResult = await this.processCommentBatch(secondHalf, results);
        return {
    ...
    if (!response && commentBatch.length > 1) {
      const mid = Math.ceil(commentBatch.length / 2);
      const firstHalf = commentBatch.slice(0, mid);
      const secondHalf = commentBatch.slice(mid);
      console.warn(
