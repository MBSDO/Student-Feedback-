...
export class Comment extends DBEntity {
  constructor() {
    super("comments", "cid");
    this.categories = null; // Add categories property
  }
  ...
  async processCommentBatch(commentBatch, results) {
    ...
    if (!response) {
      console.warn("⚠️ Skipping batch due to null response.");
      return;
    }
...
CommentRouter.post("/:cid/*action", async (req, res, next) => {
  try {
    req.comment = new Comment();
    await req.comment.Get(req.params.cid);
    req.report = new Report();
    await req.report.Get(req.comment.rid);
    if (
      req.session.user === undefined ||
      req.session.user.uid !== req.report.uid
    ) {
      res.status(403).json({ error: "Permission denied." });
      return;
    }
