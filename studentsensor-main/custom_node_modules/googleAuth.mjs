...
async function GetAccountData(accessToken) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(5000, () => {
      reject({
        status: "error",
        error: "Connection timeout: Unable to connect to openidconnect.googleapis.com",
      });
    });
  ...
  try {
    ...
    if (
      ...
      if (accountData?.sub) {
        ...
        let uid = await new User().GetIDFromSetting("google_sub", accountData.sub);
        ...
        console.log("👤 Account Data (userinfo):", accountData);
        console.log("✅ User session established:", req.session.user?.uid);

        return res.redirect("/");
      } else {
        console.error("OAuth profile fetch succeeded but missing `sub`:", accountData);
      }
    } else {
      console.error("Token exchange did not return a usable access token:", tokenResponse);
    }
