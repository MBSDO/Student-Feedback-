...
export async function OpenAISendBatch(batch, codebook) {
  ...
  if (!outputText) {
    ...
    console.warn("⚠️ OpenAI returned empty output. Falling back to codebook-only themes.");
    return null;
  }
  ...
  console.warn("⚠️ OpenAI returned unparsable JSON payload. Falling back to codebook-only themes.");
  return null;
}
...
Use ONLY the categories in the codebook. Assign 1–3 categories per comment.
...
`;
  ...
  prompt += `
Edge-case guidance (apply only when category match is not obvious):
- Prefer closest-fit category over [] when a clear learning signal exists.
- Confidence discussing content -> instructional clarity.
- Direct/toughening instructor style -> professor demeanor.
- High-stakes readiness/motivation -> student engagement.
- "Why" framing or human impact of systems -> course content relevance.
`;
