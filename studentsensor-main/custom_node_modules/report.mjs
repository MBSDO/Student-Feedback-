...
ReportRouter.post("/upload", requireAuth, async (req, res) => {
  try {
    ...
    // ✅ Progress update: saving comments
    ...
    // ✅ Progress update: comparing with codebook
    progressMap.set(report.rid, {
      percent: 30,
    ...
    // ✅ NEW: Process themes for all comments before summary generation
    console.log(
    ...
    for (const row of commentRows) {
      try {
        ...
        console.warn(
          `⚠️ Failed to process categories for comment ${row.cid}:`,
          err.message
        );
        failedCount++;
      }
    ...
    if (fsSync.existsSync(recodedPath)) {
      ...
      for (const entry of recoded) {
        ...
        try {
          ...
          console.warn(`⚠️ Failed to save categories for comment ${entry.cid}:`, err.message);
        }
      }
    }
    ...
    if (summary) {
      ...
      console.log(`⚠️ [WARNING] No summary generated for report ${report.rid}`);
    }
    ...
    if (codebook_json) {
      try {
        ...
        console.warn("⚠️ Failed to parse codebook_json:", parseErr.message);
        codebook = {};
      }
    }
    ...
    // Update progress map with error state if report was created
    if (typeof report !== 'undefined' && report && report.rid) {
      progressMap.set(report.rid, {
        percent: 0,
...
export async function summarizeReportThemes(rid) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      console.warn(`⚠️ Failed to parse categories for comment: ${err.message}`);
      // Fallback: try comma-separated string
      categories = row.categories
        .split(",")
        .map((t) => t.trim())
...
async function prepareSummaryData(rid) {
  ...
  for (const row of rows) {
    if (!row.categories) {
      console.log(
        `🔍 [DEBUG] Comment has no categories: "${row.text.substring(
          0,
          50
        )}..."`
      );
