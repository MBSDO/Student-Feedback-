console.log("🔧 report.mjs is executing...");
...
// ✅ Global in-memory progress tracker with timestamps for ETA calculation
const progressMap = new Map();
const canceledUploads = new Map();
const uploadLogPrefix = "[upload]";
const PROGRESS_HEARTBEAT_MS = 5000;
const PYTHON_PIPELINE_TIMEOUT_MS = Number(
  process.env.PYTHON_PIPELINE_TIMEOUT_MS || 180000,
);
...
function buildFallbackSummary(themeCounts) {
  ...
  const lines = ["Top themes identified from student feedback:"];
  for (const [theme, count] of entries) {
    lines.push(`- ${theme}: ${count} mention${count === 1 ? "" : "s"}`);
  }
  return lines.join("\n");
}

function isUploadCanceled(rid) {
  return canceledUploads.has(rid);
}

function throwIfUploadCanceled(rid) {
  if (isUploadCanceled(rid)) {
...
async function deleteReportArtifacts(rid) {
  const tempDir = path.resolve(global.__basedir, "tmp");
  const tempPaths = [
    sanitizeFilePath(`report_${rid}.csv`),
    sanitizeFilePath(`themes_summary_${rid}.json`),
    sanitizeFilePath(`summary_input_${rid}.json`),
  ]
    .filter(Boolean)
    .map((fileName) => path.resolve(tempDir, fileName));

  await db.query(
    `DELETE FROM comments_settings
     WHERE cid IN (SELECT cid FROM comments WHERE rid = $1)`,
    [rid]
  );
  ...
  for (const filePath of tempPaths) {
    if (!filePath.startsWith(tempDir)) continue;
    try {
      await fs.unlink(filePath);
    } catch (error) {
      if (error?.code !== "ENOENT") {
        console.warn(`⚠️ Failed to remove temp file ${filePath}:`, error?.message || error);
      }
    }
  }
}
...
function normalizeParsedRow(row) {
  ...
  const onlyFirstCellHasContent =
    cells.length === 1 || cells.slice(1).every((cell) => cell.trim().length === 0);
  if (!onlyFirstCellHasContent) {
    return cells;
  }

  const firstCell = cells[0] || "";
  for (const delimiter of CSV_CANDIDATE_DELIMITERS) {
    if (!firstCell.includes(delimiter)) continue;
    const splitCells = firstCell.split(delimiter).map((cell) => cell.trim());
    if (splitCells.length >= 4) {
      return splitCells;
    }
  }
...
function extractCommentsFromRecords(records) {
  return records
    .slice(1)
...
function parseCSVRecords(csvText) {
  ...
  for (const delimiter of CSV_CANDIDATE_DELIMITERS) {
    try {
      ...
      const comments = extractCommentsFromRecords(normalizedRecords);
      const rowsWithCommentColumn = normalizedRecords.filter((row) => row.length >= 4).length;

      attempts.push({
        delimiter,
        normalizedRecords,
        comments,
        rowsWithCommentColumn,
      });
    } catch {
  ...
  const bestAttempt = attempts.sort((a, b) => {
    ...
    if (b.rowsWithCommentColumn !== a.rowsWithCommentColumn) {
      return b.rowsWithCommentColumn - a.rowsWithCommentColumn;
    }
    return b.normalizedRecords.length - a.normalizedRecords.length;
  })[0];

  if (!bestAttempt || bestAttempt.normalizedRecords.length === 0) {
...
async function getMissingCategoryCommentIds(rid) {
  return db.query(
    `SELECT c.cid
     FROM comments c
...
// ✅ New: Progress polling endpoint with ETA calculation
ReportRouter.get("/progress/:rid", requireAuth, async (req, res) => {
  try {
    ...
    if (ownerRows[0].uid !== req.session.user.uid) {
    ...
    if (progress) {
      ...
      const updatedAtMs = Number(progress.updated_at || progress.startTime || Date.now());
      response.heartbeat_age_seconds = Math.max(
        0,
...
async function processUploadInBackground({
  ...
  const startProgressHeartbeat = () => {
    ...
    heartbeatTimer = setInterval(() => {
      const current = progressMap.get(report.rid);
      if (!current) return;
      if (current.percent >= 100 || current.state === "error") return;
  ...
  try {
    throwIfUploadCanceled(report.rid);
    ...
    for (const text of comments) {
      ...
      try {
        ...
        console.warn("⚠️ Failed to insert comment:", err.message);
      }
    }
    ...
    console.log(
      `🔍 [DEBUG] Processing categories for all comments in report ${report.rid}`
    );
    ...
    for (const row of commentRows) {
      ...
      try {
        ...
        console.warn(
          `⚠️ Failed to load comment ${row.cid} for batch processing:`,
          err.message
        );
        loadFailedCount++;
      }
    ...
    const batchResult = await batchProcessor.ProcessBatch(commentEntities, {
      shouldCancel: () => isUploadCanceled(report.rid),
    ...
    // Simple second pass: rerun only comments that still have no categories.
    try {
      const missingAfterFirstPass = await getMissingCategoryCommentIds(report.rid);
      throwIfUploadCanceled(report.rid);
      if (missingAfterFirstPass.length > 0) {
        logUpload("warn", "stage_rerun_missing_started", {
          trace_id: traceId,
          rid: report.rid,
          missing_count: missingAfterFirstPass.length,
        });
        progressMap.set(report.rid, {
          trace_id: traceId,
          percent: 52,
          message: `Re-running uncategorized comments... (${missingAfterFirstPass.length} remaining)`,
          state: "generating_tags",
          startTime,
          updated_at: Date.now(),
        });

        const rerunEntities = [];
        for (const row of missingAfterFirstPass) {
          throwIfUploadCanceled(report.rid);
          const comment = new Comment();
          try {
            await comment.Get(row.cid);
            rerunEntities.push(comment);
          } catch (err) {
            console.warn(
              `⚠️ Failed to load comment ${row.cid} for automatic rerun:`,
              err?.message || err,
            );
          }
        }

        if (rerunEntities.length > 0) {
          const rerunProcessor = new Comment();
          const rerunResult = await rerunProcessor.ProcessBatch(rerunEntities, {
            shouldCancel: () => isUploadCanceled(report.rid),
          });
          throwIfUploadCanceled(report.rid);
          const remainingAfterRerun = await getMissingCategoryCommentIds(report.rid);
          logUpload("info", "stage_rerun_missing_completed", {
            trace_id: traceId,
            rid: report.rid,
            rerun_attempted: rerunEntities.length,
            rerun_processed: rerunResult.processedCount,
            rerun_failed: rerunResult.failedCount,
            remaining_missing: remainingAfterRerun.length,
          });
        }
      }
    } catch (rerunErr) {
      // Do not fail the entire pipeline if second-pass rerun has issues.
      console.warn(
        "⚠️ Automatic rerun for missing categories failed:",
        rerunErr?.message || rerunErr,
      );
    }
    ...
    console.log(
      `🔍 [DEBUG] Category processing complete: ${processedCount} successful, ${failedCount} failed`
    );
    ...
    if (fsSync.existsSync(recodedPath)) {
      ...
      for (const entry of recoded) {
        ...
        try {
          ...
          console.warn(
            `⚠️ Failed to save categories for comment ${entry.cid}:`,
            err.message
          );
        }
      }
    ...
    if (normalizedSummary) {
      console.log(
        `🔍 [DEBUG] Saving summary for report ${report.rid}:`,
        normalizedSummary.substring(0, 200) + "..."
      );
      settingsToSave.summary_text = normalizedSummary;
    } else {
      console.log(
        `⚠️ [WARNING] No summary generated for report ${report.rid}; using fallback summary from theme counts`
      );
      settingsToSave.summary_text = buildFallbackSummary(themeCounts);
    }
    ...
    if (err?.code === "UPLOAD_CANCELED" || err?.message === "Upload canceled") {
      logUpload("warn", "pipeline_canceled", {
        trace_id: traceId,
        uid,
        rid: report.rid,
        duration_ms: Date.now() - startTime,
      });
      await deleteReportArtifacts(report.rid);
      progressMap.delete(report.rid);
      canceledUploads.delete(report.rid);
      return;
    }
...
ReportRouter.post("/upload", requireAuth, async (req, res) => {
  try {
    ...
    try {
      const parsedCSV = parseCSVRecords(csv_text);
      records = parsedCSV.normalizedRecords;
      comments = parsedCSV.comments;
      detectedDelimiter = parsedCSV.delimiter;
    } catch (parseError) {
      console.error("❌ CSV parsing failed:", parseError);
      return res.status(400).json({
        error: "Failed to parse CSV file. Please ensure it uses a supported delimiter.",
      });
    }
    ...
    report = new Report();
    report.name = `${validatedProfessor} – ${validatedCourseCode} (${validatedSemester})`;
    await report.Create(uid);
...
ReportRouter.post("/:rid/cancel", requireAuth, async (req, res) => {
  try {
    const rid = validateId(req.params.rid);
    if (!rid) {
      return res.status(400).json({ error: "Invalid report ID" });
    ...
    const currentProgress = progressMap.get(rid);
    if (currentProgress?.state === "complete") {
      return res.status(409).json({ error: "Upload already completed" });
    }

    canceledUploads.set(rid, {
      canceled_at: Date.now(),
      uid: req.session.user.uid,
    });
    progressMap.set(rid, {
      trace_id: currentProgress?.trace_id || null,
      percent: currentProgress?.percent || 0,
      message: "Canceling upload...",
      state: "canceled",
      startTime: currentProgress?.startTime || Date.now(),
      updated_at: Date.now(),
    });

    logUpload("warn", "cancel_requested", {
      rid,
      uid: req.session.user.uid,
      trace_id: currentProgress?.trace_id || null,
      state: currentProgress?.state || null,
    });

    return res.json({ success: true, canceling: true });
  } catch (err) {
...
ReportRouter.post("/:rid/reprocess-missing", requireAuth, async (req, res) => {
  try {
    const rid = validateId(req.params.rid);
    if (!rid) {
      return res.status(400).json({ error: "Invalid report ID" });
    ...
    const missingRows = await getMissingCategoryCommentIds(rid);

    if (missingRows.length === 0) {
      return res.json({
        success: true,
        rid,
        message: "No missing comment categories found.",
        total_missing: 0,
        processed: 0,
        failed: 0,
      });
    ...
    for (const row of missingRows) {
      ...
      try {
        ...
        console.warn(
          `⚠️ Failed to load comment ${row.cid} for reprocess-missing:`,
          err?.message || err
        );
      }
    }

    const processor = new Comment();
    const batchResult = await processor.ProcessBatch(commentEntities);

    // Keep report-level theme summary synced after backfill.
    const themeCounts = await summarizeReportThemes(rid);
    await report.SaveSetting("themes_summary_json", JSON.stringify(themeCounts));
    let summaryRefreshed = false;
    let summaryLength = 0;
    try {
      const jsonPath = await prepareSummaryData(rid);
      const { summary } = await runPythonCodebookPipeline(jsonPath, rid);
      const normalizedSummary = typeof summary === "string" ? summary.trim() : "";
      const finalSummary = normalizedSummary || buildFallbackSummary(themeCounts);
      await report.SaveSetting("summary_text", finalSummary);
      summaryRefreshed = true;
      summaryLength = finalSummary.length;
    } catch (summaryErr) {
      console.warn(
        "⚠️ Failed to refresh summary_text during reprocess-missing:",
    ...
    return res.json({
      ...
      loaded: commentEntities.length,
      processed: batchResult.processedCount,
      failed: batchResult.failedCount,
      refreshed_theme_count: Object.keys(themeCounts || {}).length,
      summary_refreshed: summaryRefreshed,
      summary_length: summaryLength,
    });
  } catch (err) {
    console.error("Failed to reprocess missing categories:", err);
    const errorResponse = safeErrorResponse(
      err,
      process.env.NODE_ENV === "development"
    );
...
export async function summarizeReportThemes(rid) {
  console.log(`🔍 [DEBUG] Summarizing categories for report ${rid}`);
  ...
  console.log(
    `🔍 [DEBUG] Found ${rows.length} comments for category summarization`
  );
  ...
  for (const row of rows) {
    if (!row.categories) {
      // console.log(`🔍 [DEBUG] Comment has no categories for summarization`);
      continue;
    }

    const categories = parseCategoryList(row.categories).filter(
      (category) => String(category || "").toLowerCase() !== "uncategorized"
    );
    ...
    // console.log(`🔍 [DEBUG] Processing categories for summarization: [${categories.join(', ')}]`);
  ...
  console.log(
    `🔍 [DEBUG] Category summarization complete: ${commentsWithCategories}/${rows.length} comments had categories`
  );
  console.log(`🔍 [DEBUG] Final category counts:`, themeCounts);
...
function runPythonCodebookPipeline(jsonPath, reportId) {
  return new Promise((resolve, reject) => {
    console.log(
      `🔍 [DEBUG] Starting Python pipeline for report ${reportId} with data from ${jsonPath}`
    );

    // ✅ Default to system python3 (for Render)
    let pythonPath = "python3";

    // ✅ Use local venv python if it exists
    ...
    if (fsSync.existsSync(localVenvPath)) {
      ...
      console.log(`🔍 [DEBUG] Using local venv python: ${pythonPath}`);
    } else {
    ...
    const timeoutId = setTimeout(() => {
      ...
      reject(
        new Error(
          `Python pipeline timed out after ${PYTHON_PIPELINE_TIMEOUT_MS}ms`,
        ),
      );
    }, PYTHON_PIPELINE_TIMEOUT_MS);
    ...
    proc.on("close", (code) => {
      ...
      if (code !== 0) {
        console.error(`❌ [ERROR] Python pipeline failed with code ${code}`);
      ...
      try {
        ...
        // console.log(`🔍 [DEBUG] Python pipeline result:`, result);
        resolve(result);
      } catch (err) {
        console.error(
...
async function prepareSummaryData(rid) {
  console.log(`🔍 [DEBUG] Preparing summary data for report ${rid}`);
  ...
  console.log(`🔍 [DEBUG] Found ${rows.length} comments for report ${rid}`);
  ...
  for (const row of rows) {
    if (!row.categories) {
      ...
      console.log(`🔍 [DEBUG] Comment has no categories: "${textPreview}..."`);
      continue;
    }
    ...
    // console.log(`🔍 [DEBUG] Comment categories: [${categories.join(', ')}] for text: "${row.text.substring(0, 50)}..."`);
  ...
  console.log(
    `🔍 [DEBUG] Summary stats: ${commentsWithCategories}/${rows.length} comments have categories, ${totalCategories} total category occurrences`
  );
  console.log(`🔍 [DEBUG] Category counts:`, themeCounts);
  console.log(`🔍 [DEBUG] Comments by category:`, Object.keys(commentsByTheme));
  ...
  const data = {
    ...
    console.log(`🔍 [DEBUG] Summary data written to: ${tempPath}`);
    return tempPath;
}
